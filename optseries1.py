...
#########
x = x_uniform1(xlim, n)
r0 = doplot(x, b0, sd, 'Uniform, unique X', False)
mr00 = median(r0[0])
mr01 = median(r0[1])
mr02 = median(r0[2])
print "Uniform:", median(r0[0]), median(r0[1]), median(r0[2])
...
def compoundexp(pars, x):
    ...
    for par in pars:
        try:
            prob += par[2]*exp(-(x-par[0])**2/(2*par[1]**2))
        except NameError:
            prob = par[2]*exp(-(x-par[0])**2/(2*par[1]**2))
        h += par[2]
    prob = prob / h
...
def findcompound(p0):
    ...
    # b2 = p0[3]
    # c2 = p0[4]
    # pars = ((-a, b, 1), (a, b, 1), (-a2, b2, c2), (a2, b2, c2))
    x = []
    while len(x) < n:
        xt = random.uniform(xlim[0]*g, xlim[1]*g)
        xt = random.uniform(xlim[0]*g, 0)
        y = random.uniform(0, len(pars))
        if (y < compoundexp(pars, xt)):
            x.append(xt)
    x = sort(array(x))
    r1 = doplot(x, b0, sd, 'Compound', False)
    mr0 = median(r1[0])
    mr1 = median(r1[1])
    mr2 = median(r1[2])
    print "Fitted :", mr0, mr1, mr2, p0
    return mr1

ng = 25
al = linspace(0, 1, ng)
bl = linspace(0.1, 2, ng)
X, Y = meshgrid(al, bl)
Z = zeros((ng, ng))
for i in xrange(ng):
    for j in xrange(ng):
        p0 = (X[i,j], Y[i,j])
        Z[i, j] = mr01/findcompound(p0)
CS = pl.contour(X, Y, Z, 15)
pl.clabel(CS, inline=1, fontsize=10)
pl.show()
# p0 = (0.6, 0.2, 0.4, 0.2, 2)
...
# c2 = p0[4]
# pars = ((-a, b, 1), (a, b, 1), (-a2, b2, c2), (a2, b2, c2))
# pl.plot(x, compoundexp(pars, x))
# pl.show()

# popt = fmin(findcompound, p0)
# print popt
