...
lorentz = lambda x, x0, g, A: A*g/((x-x0)**2+g**2)
...
def doplot(x, b0, sd, title, dofig=False):
    ...
    return [x0_1, g_1, a_1, x0_1r, g_1r, a_1r]
...
bbin = linspace(-0.1, 0.1, 201)

x = x_uniform1(xlim, n)
r1 = doplot(x, b0, sd, 'Uniform, unique X', False)
pl.figure(1)
nn1, bins1, patches = pl.hist(r1[parmn], bbin, histtype='stepfilled')
h1 = diff(bins1)[0]
pl.figure(2)
pl.plot(bins1[1:]-h1/2, nn1/rep, label='Uniform')
# pl.show()
# mr00 = mean(r1[0])
# mr01 = mean(r1[1])
# mr02 = mean(r1[2])
# print "Uniform:", mr00, mr01, mr02
...
# Total
# a = 0.774
# xa = array([[-a-x0, 0, a-x0],[10, 10, 10]])

# x0 optimum
a = 0.576
c = 0.05
xa = array([[-a-x0, 0-x0, a-x0],[48, 4, 48]])
# xa = array([[-a-x0-c, -a-x0, -a-x0+c, 0, a-x0-c, a-x0, a-x0+c],[1,13,1, 0, 1, 13, 1]])

# Gamma
# a = 1.18
# xa = array([[-a/g, 0, a/g],[35, 30, 35]])

# I
# a = 1
# xa = array([[-a, 0, a],[49, 2, 49]])

x = array([])
for i, newx in enumerate(xa[0,:]):
    xnum = xa[1,i]
    x = append(x, array([newx]*xnum))
r2 = doplot(x, b0, sd, 'Optimal', False)
pl.figure(1)
nn1, bins1, patches = pl.hist(r2[parmn], bbin, histtype='stepfilled')
h2 = diff(bins1)[0]
pl.figure(2)
pl.plot(bins1[1:]-h2/2, nn1/rep, '--', label='optimized')
pl.legend(loc='best')
...
# mr21 = mean(r2[1])
# mr22 = mean(r2[2])
# print "Opt (ratio):", mr20/mr00, mr21/mr01, mr22/mr02
# for i in xrange(3):
#     print mean(array(r2[i])/array(r1[i]))
...
#     mr51 = median(r5[1])
#     mr52 = median(r5[2])
#     print "Predicted:", mr50/mr00, mr51/mr01, mr52/mr02
#     return (mr50, mr51, mr52)

# a = 1.1
# b = 0.1
# ret = predict(a, b, [0, 1, 1])

# ### Mis-specified X0
# x0l = linspace(-1, 1, 31)
# x0p = array([])
# for x0 in x0l:
#      x0p = append(x0p, predict(a, b, [x0, 1, 1])[0])

# pl.plot(x0l, x0p/mr0)
# pl.plot([x0l[0], x0l[-1]], [1, 1], 'k--')
# pl.show()

# gl = linspace(0.1, 2, 31)
# gp = array([])
# for g0 in gl:
#      gp = append(gp, predict(a/g0, b/g0, [0, 1, 1])[1])

# pl.plot(gl, gp/mr01)
# pl.plot([gl[0], gl[-1]], [1, 1], 'k--')
# pl.show()
